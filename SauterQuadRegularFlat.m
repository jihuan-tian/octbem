function ret = SauterQuadRegularFlat(kernel_function,
				     kx_basis_function,
				     ky_basis_function,
				     ...
				     qpts, qwts)
  ## SauterQuadRegularFlat - Perform the regular integration using the
  ## method in Sauter's BEM, where the quadrangle is flat. The
  ## integral is
  ## \f[
  ...
  k3 = @(kx_ky_area_coord) H_loc(kx_ky_area_coord(:,1:2), kx_ky_area_coord(:,3:4)) * k_loc(kx_ky_area_coord(:,1:2), kx_ky_area_coord(:,3:4));
  
  ## Apply 4d Gauss-Legendre quadrature rule to the integrand.
  ret = ApplyQuadRule(k3, qpts, qwts);
endfunction
