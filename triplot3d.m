function h = triplot3d(mesh_cells, mesh_nodes, varargin)
  ## triplot3d - Plot a list of triangles embedded in 3D space.
  ## @param mesh_cells A list of triangular cells, each of which is stored as an
  ...
  cell_geom_order = TriaGeometryOrder(number_of_nodes_in_cell);

  ## Get the corner node indices of the triangular cells.
  mesh_cells_corner_only = mesh_cells(:, GetTriaCornerNodeIndices(cell_geom_order));
  ## Generate the local node indices to form a loop of the triangle corner
  ## nodes.
  corner_node_indices_loop = mesh_cells_corner_only(:, [1, 2, 3, 1]);
