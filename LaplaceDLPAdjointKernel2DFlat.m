function ret = LaplaceDLPAdjointKernel2DFlat(x, y, nx, ny)
  ## LaplaceDLPAdjointKernel2DFlat - Adjoint lIntegration kernel for the double
  ...
  if (number_of_points != size(y, 1))
    ...
    for m = 1:number_of_points
      ret(m) = inner_prod(x(m, :) - y(m, :), NormalizeVect(nx)) / 2.0 / pi / norm(x(m, :) - y(m, :))^2;
    endfor
  endif
endfunction
