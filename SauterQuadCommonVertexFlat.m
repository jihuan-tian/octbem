function ret = SauterQuadCommonVertexFlat(kernel_function, 
					  kx_basis_function,
					  ky_basis_function,
					  ...
					  qpts, qwts)
  ## SauterQuadCommonVertexFlat - Perform the common vertex integration
  ...
  k3_tilde = @(p) p(:,1).^3 .* (
	     k3([p(:,1), p(:,1) .* p(:,2)], [p(:,1) .* p(:,3), p(:,1) .* p(:,4)]) + 
	     k3([p(:,1) .* p(:,2), p(:,1)], [p(:,1) .* p(:,3), p(:,1) .* p(:,4)]) +
	     k3([p(:,1) .* p(:,2), p(:,1) .* p(:,3)], [p(:,1), p(:,1) .* p(:,4)]) +
	     k3([p(:,1) .* p(:,2), p(:,1) .* p(:,3)], [p(:,1) .* p(:,4), p(:,1)]));
