function ret = SauterQuadSamePanelFlat(kernel_function, 
				       kx_basis_function,
				       ky_basis_function,
				       ...
				       qpts, qwts)
  ## SauterQuadSamePanelFlat - Perform the same panel integration
  ...
  k3_tilde = @(p) p(:,1) .* (1 - p(:,1)) .* (1 - p(:,1) .* p(:,2)) .* ...
	     (k3([(1 - p(:,1)) .* p(:,4), ...
		  (1 - p(:,1) .* p(:,2)) .* p(:,3)],
		 [p(:,1) + (1 - p(:,1)) .* p(:,4), ...
		  p(:,1) .* p(:,2) + (1 - p(:,1) .* p(:,2)) .* p(:,3)]) + ...
	      k3([(1 - p(:,1) .* p(:,2)) .* p(:,3), ...
		  (1 - p(:,1)) .* p(:,4)],
		 [p(:,1) .* p(:,2) + (1 - p(:,1) .* p(:,2)) .* p(:,3), ...
		  p(:,1) + (1 - p(:,1)) .* p(:,4)]) + ...
